...
def train_neural_network(X, Y):
    ...
    cost_func = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(logits=predict, labels=Y))#tf.multiply(0,a)
    optimizer = tf.train.AdamOptimizer().minimize(cost_func)  # learning rate 默认 0.001
    save_path = "C:/Users/Jz Chai/PycharmProjects/TFexercice/Ex1/tmp/model.ckpt"
    epochs = 3
    with tf.Session() as session:
        #writer = tf.summary.FileWriter('./graphs', session.graph)
        saver=tf.train.Saver()
        print('e')
        if os.path.exists("C:/Users/Jz Chai/PycharmProjects/TFexercice/Ex1/tmp/model.ckpt.index"):
        ...
        for epoch in range(epochs):
            ...
            if epoch_loss<old_epoch:
                old_epoch = epoch_loss
